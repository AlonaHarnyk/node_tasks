...
- Якщо контакт за `id` знайдений і видалений, повертає json формату `{"message": "Сontact deleted"}` зі статусом `200`
- Якщо контакт за `id` не знайдено, повертає json формату `{"message": "Not found"}` зі статусом `404`
...
- Якщо запит на оновлення здійснено без передачі в `body` хоча б одного поля, повертає json формату `{"message": "Body must have at least one field"}` і статусом `400`.
